const RUNNERS_KEY = "ariaChatgptTabRunners";
const ALARM_NAME = "aria-chatgpt-runner-watchdog";
...
function saveRunners() {
  try {
    ...
    console.warn("[ARIA ChatGPT Background] save failed", err);
  }
}
  ...
  try {
    ...
    chrome.tabs.update(tabId, { autoDiscardable }, () => {
      if (chrome.runtime.lastError) {
        console.warn("[ARIA ChatGPT Background] tab discard setting failed", chrome.runtime.lastError.message);
      }
    });
  } catch (err) {
    console.warn("[ARIA ChatGPT Background] tab discard setting failed", err);
  }
}
...
async function ensureOffscreenDocument() {
  try {
    ...
    console.warn("[ARIA ChatGPT Background] offscreen unavailable", err);
  } finally {
...
function ensureAlarm() {
  try {
    ...
    console.warn("[ARIA ChatGPT Background] alarm failed", err);
  }
}
...
function loadRunners() {
  try {
    ...
    console.warn("[ARIA ChatGPT Background] load failed", err);
  }
}

function registerRunner(sender, message) {
  const tab = sender && sender.tab ? sender.tab : null;
  if (!tab || tab.id === undefined || tab.id === null || !isChatgptUrl(tab.url || message.url)) {
    return { ok: false, error: "No ChatGPT tab found for registration." };
  }
...
function pump(force = false) {
  ...
  runners.forEach((runner, tabId) => {
    ...
    if (!force && runner.lastTickAt && t - runner.lastTickAt < wakeEveryMs) {
      return;
    }
    runner.lastTickAt = t;
